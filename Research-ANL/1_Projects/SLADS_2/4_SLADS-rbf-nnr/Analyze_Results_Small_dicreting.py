...
Img = misc.imread('ResultsAndData/TestingImages/' + ImagePath + '/Image_1.tif')
misc.imsave('ResultsAndData/SLADSSimulationResults/' + path + '/Original.png',Img)


## discreting measured value
from sklearn.cluster import KMeans

km = KMeans(n_clusters=3)

km.fit(MeasuredValues.reshape(-1,1))

km.labels_

MeasuredValues = np.copy(km.labels_)
...
for n in range(0,PercentageVect.shape[0]):
    ...
    print(TD)
    name = 'ResultsAndData/SLADSSimulationResults/' + path + '/Recon_' + str(PercentageVect[n]) + '_D_.png'
...
sample_index = [5, 10, 15, 20, 25, 30, 35, 40]

from matplotlib import pyplot as plt    
plt.plot(sample_index, TDvec)
plt.plot(sample_index, PSNRvec)
